...
exports.useUserTyping = function (content, setContent, lastUnsubmittedValue, lastInputValue, avatarString) {
    ...
    react_1.useEffect(function () {
        var lastEntry = content.length > 0 ? content[content.length - 1] : undefined;
        if (!lastInputValue) {
            if (lastUnsubmittedValue && (!lastEntry || !lastEntry.isUser || !lastEntry.isLoading)) {
                var newContent = __spreadArrays(content, [
                    mobx_1.observable({
                        isUser: true,
                        isLoading: true,
                        message: getAsMessageContent_1.getAsMessageContent(lastUnsubmittedValue),
                        avatar: userAvatar,
                        id: uuid_1.uuid()
                    })
                ]);
        ...
        if (lastInputValue) {
            if (lastEntry && lastEntry.isUser && lastEntry.isLoading) {
                ...
                lastEntry.message = getAsMessageContent_1.getAsMessageContent(lastInputValue);
            }
                ...
                setContent(__spreadArrays(content, [
                    mobx_1.observable({
                        isUser: true,
                        isLoading: false,
                        message: getAsMessageContent_1.getAsMessageContent(lastInputValue),
                        avatar: userAvatar,
                        id: uuid_1.uuid()
                    })
                ]));
