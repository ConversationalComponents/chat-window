...
const getAsMessageContent = (v: MessageContent[] | string) => (typeof v === "string" ? [{text: v}] : v);

export const useUserTyping = (
    content: ChatEntry[],
    setContent: (content: ChatEntry[]) => void,
    lastUnsubmittedValue: MessageContent[] | string,
    lastInputValue: MessageContent[] | string,
    avatarString?: string
) => {
    ...
    useEffect(() => {
            ...
            if (lastUnsubmittedValue && (!lastEntry || !lastEntry.isUser || !lastEntry.isLoading)) {
                const newContent = [
                    ...
                    observable({
                        isUser: true,
                        isLoading: true,
                        message: getAsMessageContent(lastUnsubmittedValue),
                        avatar: userAvatar,
                        id: uuid()
                    })
                ];
        ...
        if (lastInputValue) {
            if (lastEntry && lastEntry.isUser && lastEntry.isLoading) {
                ...
                lastEntry.message = getAsMessageContent(lastInputValue);
                ...
                setContent([
                    ...
                    observable({
                        isUser: true,
                        isLoading: false,
                        message: getAsMessageContent(lastInputValue),
                        avatar: userAvatar,
                        id: uuid()
                    })
                ]);
